...
| `client add`         | `--token <jwt token>` `--server-url <url>`, `[--file <путь_к_файлу>]`, `[--interactive]`                               | Добавление новых данных/секретов из файла или интерактивно            |
| `client get`         | `--token <jwt token>`,`--server-url <url>`, `[--secret-id <идентификатор>]`                                            | Получение конкретных данных/секрета с сервера                         |
