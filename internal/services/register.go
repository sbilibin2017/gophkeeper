...
// RegisterHTTP отправляет HTTP-запрос для регистрации нового пользователя.
...
func RegisterHTTP(
	...
	if resp.StatusCode() != http.StatusOK && resp.StatusCode() != http.StatusCreated {
		return "", fmt.Errorf("registration failed: status %d, response: %s", resp.StatusCode(), resp.String())
	}

	return result.Token, nil
}

// RegisterGRPC вызывает gRPC метод Register для регистрации нового пользователя.
