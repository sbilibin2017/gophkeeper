...
func TestNewClientConfig_DBError(t *testing.T) {
	origDBNew := dbNew
...
func TestNewClientConfig_DefaultSuccess(t *testing.T) {
	cleanupDBFile(t)
	cfg, err := NewClientConfig()
...
func TestWithHTTPClient_WithTLSAndToken(t *testing.T) {
	...
	httpClientNew = func(url string, opts ...httpClient.Opt) (*resty.Client, error) {
		...
		// Can't check opts internals easily but we know two options passed
		return resty.New(), nil
	}
