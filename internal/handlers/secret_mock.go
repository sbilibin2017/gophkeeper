...
func (m *MockSecretTokenDecoder) Parse(tokenString string) (string, string, error) {
	m.ctrl.T.Helper()
...
func (m *MockSecretWriter) Save(ctx context.Context, secretID, userID, secretName, secretType string, encryptedPayload, nonce []byte, meta string) error {
	m.ctrl.T.Helper()
	ret := m.ctrl.Call(m, "Save", ctx, secretID, userID, secretName, secretType, encryptedPayload, nonce, meta)
	ret0, _ := ret[0].(error)
	return ret0
}

// Save indicates an expected call of Save.
func (mr *MockSecretWriterMockRecorder) Save(ctx, secretID, userID, secretName, secretType, encryptedPayload, nonce, meta interface{}) *gomock.Call {
	mr.mock.ctrl.T.Helper()
	return mr.mock.ctrl.RecordCallWithMethodType(mr.mock, "Save", reflect.TypeOf((*MockSecretWriter)(nil).Save), ctx, secretID, userID, secretName, secretType, encryptedPayload, nonce, meta)
}
...
func (m *MockSecretReader) Get(ctx context.Context, userID, secretName string) (*models.SecretDB, error) {
	m.ctrl.T.Helper()
	ret := m.ctrl.Call(m, "Get", ctx, userID, secretName)
...
func (mr *MockSecretReaderMockRecorder) Get(ctx, userID, secretName interface{}) *gomock.Call {
	mr.mock.ctrl.T.Helper()
	return mr.mock.ctrl.RecordCallWithMethodType(mr.mock, "Get", reflect.TypeOf((*MockSecretReader)(nil).Get), ctx, userID, secretName)
}
