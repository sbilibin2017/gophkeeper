...
// Dummy gRPC AuthService implementation for testing login
type testAuthServiceServer struct {
	pb.UnimplementedAuthServiceServer
}

func (s *testAuthServiceServer) Login(ctx context.Context, req *pb.AuthRequest) (*pb.AuthResponse, error) {
	return &pb.AuthResponse{Token: "dummy-grpc-login-token"}, nil
}

// Start a simple HTTP test server for login
func startTestHTTPServer(t *testing.T) (*http.Server, net.Listener) {
	...
	handler.HandleFunc("/login", func(w http.ResponseWriter, r *http.Request) {
		w.Header().Set("Authorization", "Bearer real-http-login-token")
...
// Start a simple gRPC test server for login
...
func TestNewCommand_HTTP_Integration(t *testing.T) {
	...
	err := cmd.Execute()
	assert.NoError(t, err)
	assert.Contains(t, outBuf.String(), "real-http-login-token")
}
...
func TestNewCommand_GRPC_Integration(t *testing.T) {
	...
	err := cmd.Execute()
	assert.NoError(t, err)
	assert.Contains(t, outBuf.String(), "dummy-grpc-login-token")
}
