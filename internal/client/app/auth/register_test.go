package auth

import (
	"bytes"
	"context"
	"strings"
	"testing"

	"github.com/sbilibin2017/gophkeeper/internal/client/models"
	"github.com/spf13/cobra"
	"github.com/stretchr/testify/require"
)

func TestRegisterRegisterCommand(t *testing.T) {
	// Backup original funcs and restore after test
	origHTTPFunc := registerHTTPFunc
	origGRPCFunc := registerGRPCFunc
	defer func() {
		registerHTTPFunc = origHTTPFunc
		registerGRPCFunc = origGRPCFunc
	}()

	// Mock the HTTP register func
	registerHTTPFunc = func(ctx context.Context, username, password, authURL, tlsCertFile, tlsKeyFile string) (*models.AuthResponse, error) {
		return &models.AuthResponse{Token: "mock-http-token"}, nil
	}

	// Mock the gRPC register func
	registerGRPCFunc = func(ctx context.Context, username, password, authURL, tlsCertFile, tlsKeyFile string) (*models.AuthResponse, error) {
		return &models.AuthResponse{Token: "mock-grpc-token"}, nil
	}

	root := &cobra.Command{Use: "root"}
	RegisterRegisterCommand(root)

	// Test HTTP scheme
	root.SetArgs([]string{
		"register",
		"--username", "alice",
		"--password", "pass",
		"--auth-url", "https://example.com",
		"--tls-client-cert", "cert.pem",
		"--tls-client-key", "key.pem",
	})

	outBuf := new(bytes.Buffer)
	root.SetOut(outBuf)
	root.SetErr(outBuf)

	err := root.Execute()
	require.NoError(t, err)
	output := outBuf.String()
	require.Contains(t, output, "mock-http-token")

	// Test gRPC scheme
	root.SetArgs([]string{
		"register",
		"--username", "bob",
		"--password", "pass",
		"--auth-url", "grpc://example.com",
		"--tls-client-cert", "cert.pem",
		"--tls-client-key", "key.pem",
	})
	outBuf.Reset()

	err = root.Execute()
	require.NoError(t, err)
	output = outBuf.String()
	require.Contains(t, output, "mock-grpc-token")

	// Test unsupported scheme
	root.SetArgs([]string{
		"register",
		"--username", "charlie",
		"--password", "pass",
		"--auth-url", "ftp://example.com",
		"--tls-client-cert", "cert.pem",
		"--tls-client-key", "key.pem",
	})
	outBuf.Reset()

	err = root.Execute()
	require.Error(t, err)
	require.True(t, strings.Contains(err.Error(), "unsupported auth URL scheme"))
}
