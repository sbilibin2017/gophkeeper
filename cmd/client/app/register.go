...
// newRegisterCommand creates the Cobra command for user registration.
	...
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if config.HTTPClient != nil {
				err = services.RegisterHTTP(
					ctx,
					req,
					services.WithRegisterHTTPClient(config.HTTPClient),
					services.WithRegisterHTTPEncoders(config.Encoders),
				)
				if err != nil {
					return err
				}
			...
			if config.GRPCClient != nil {
				...
				err = services.RegisterGRPC(
					ctx,
					req,
					services.WithRegisterGRPCClient(client),
					services.WithRegisterGRPCEncoders(config.Encoders),
				)
				if err != nil {
					return err
				}
